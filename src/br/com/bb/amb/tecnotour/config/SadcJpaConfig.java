...
@Configuration
public class SadcJpaConfig
{
    @Bean(name = { "transactionManagerSadc" })
    ...
    public SadcLocalContainerEntityManagerFactoryBean entityManagerFactory() throws Exception {
        try {
            ...
            factoryBean.setPersistenceUnitName("sadc");
            factoryBean.setDataSource(this.dataSource());
            this.eclipseLinkConfig(factoryBean);
            factoryBean.setPackagesToScan(new String[] { "br.com.bb.amb.sadc.modelo.entidade" });
            return factoryBean;
        }
        catch (Exception e) {
            throw e;
    ...
    public AnnotationMBeanExporter annotationMBeanExporter() {
        ...
        annotationMBeanExporter.addExcludedBean("dataSourceSadc");
        annotationMBeanExporter.setRegistrationPolicy(RegistrationPolicy.IGNORE_EXISTING);
        return annotationMBeanExporter;
    }
    
    @Bean(name = { "dataSourceSadc" })
    public DataSource dataSource() throws NamingException {
        return (DataSource)new JndiTemplate().lookup("java:comp/env/jdbc/sadc3");
    }
}